...
class BusMap extends StatefulWidget {
  final List<Map<String, dynamic>> vehicles; // each with latitude/longitude/registration_number/route_name/is_stale
...
class _BusMapState extends State<BusMap> {
  ...
  void didUpdateWidget(covariant BusMap old) {
    ...
    if (widget.selectedId != null && widget.selectedId != old.selectedId) {
      final v = _positioned.firstWhere(
        (e) => e['registration_number']?.toString() == widget.selectedId,
        orElse: () => const {},
      );
      if (v.isNotEmpty) {
        _controller.move(
          LatLng((v['latitude'] as num).toDouble(), (v['longitude'] as num).toDouble()),
          15,
        );
      }
    }
                ...
                return Marker(
                  ...
                  child: Container(
                    decoration: BoxDecoration(
                      color: isStale ? AppColors.muted : (isSelected ? AppColors.violet : AppColors.teal),
                      shape: BoxShape.circle,
                      border: Border.all(color: Colors.white, width: 2),
                      boxShadow: const [BoxShadow(color: Colors.black26, blurRadius: 4)],
                    ),
                    alignment: Alignment.center,
                    child: Text('🚌', style: TextStyle(fontSize: isSelected ? 18 : 14)),
                  ),
                );
              }).toList(),
            ),
